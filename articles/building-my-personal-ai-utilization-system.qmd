...
- GitHub keeps code state in parallel with the Drive memory flow
...
```{mermaid fig-cap="System-level flow of inputs, memory, tools, and outputs."}
flowchart LR
  ...
  subgraph IN[INPUTS]
    ...
    B[Research papers]
    C[Notes and manuals]
    D[Drafts and outputs]
  end

  IN --> GCLI[Gemini CLI plus gws\nclassify, rename, route, update context]
  ...
  KT --> OUT2[Research and analysis]
  ET --> OUT1[Design and implementation]
...
Reference implementation of the architecture, including setup notes, example folder structures, and system documentation.
:::
